...
/* --- NTSC MEGA LITE + RAINBOW PNG (v130 Build) ---
   - Optimized for Android & High-End Mobile.
   - Features: Resolution, Sharpness, Fringing, Artifacts.
   - Integrated: Rainbow PNG Sampling + Jitter + De-Dither.
*/
...
// وظائف التحويل اللوني
vec3 RGB_TO_YIQ(vec3 c) {
    return vec3(dot(c, vec3(0.2989, 0.5870, 0.1140)), 
                dot(c, vec3(0.5959, -0.2744, -0.3216)), 
...
void main() {
    ...
    vec3 c_left  = texture(Texture, uv - ps * de_dither).rgb;
    vec3 c_right = texture(Texture, uv + ps * de_dither).rgb;
    ...
    vec2 chrL = RGB_TO_YIQ(texture(Texture, uv - vec2(b_dist, 0.0)).rgb).gb;
    vec2 chrR = RGB_TO_YIQ(texture(Texture, uv + vec2(b_dist, 0.0)).rgb).gb;
    ...
    // 5. SHARPENING & MIST (معالجة السطوع)
    float yC = res.r;
    ...
    // 6. RAINBOW PNG SAMPLING (محاكاة تداخل الألوان)
    ...
    vec3 pngCol = texture(PNG2, fract(pUV)).rgb;
    pngCol = mix(vec3(dot(pngCol, vec3(0.299, 0.587, 0.114))), pngCol, rb_sat);

    // 7. FINAL ASSEMBLY (إعادة التحويل إلى RGB)
    vec3 final_rgb;
    ...
    // حساب قناع الرينبو بناءً على تغير السطوع (التنقيط/Dithering)
