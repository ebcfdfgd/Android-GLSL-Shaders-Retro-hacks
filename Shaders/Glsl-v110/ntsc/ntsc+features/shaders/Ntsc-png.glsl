...
uniform float rb_opacity, rb_threshold, rb_zoom, rb_speed, rb_tilt, rb_sat, de_dither, red_persistence, NOISE_STR, tv_mist, COL_BLEED, horiz_sharp;
...
void main() {
    ...
    // 4. CHROMA, FRINGING & PERSISTENCE (Logic 1010)
    vec2 mixed_chroma = res.gb;
    float total_bleed = COL_BLEED + (fring * 5.0);
    
    if(total_bleed > 0.0 || red_persistence > 0.0) {
        float b_dist = ps.x * (total_bleed + 0.001);
        ...
        // سيلان الأحمر جهة اليمين
        if(red_persistence > 0.0) {
            float smear = mix(res.g, chrL.x, 0.4 * red_persistence);
    ...
    if(total_sharp > 0.0) {
        final_y = yC + (yC * 2.0 - yL - yR) * total_sharp;
    }
    ...
    // 6. FINAL ASSEMBLY & RAINBOW
    vec3 result_yiq_to_rgb;
    result_yiq_to_rgb.r = final_y + 0.9563 * mixed_chroma.x + 0.6210 * mixed_chroma.y;
    result_yiq_to_rgb.g = final_y - 0.2721 * mixed_chroma.x - 0.6474 * mixed_chroma.y;
    result_yiq_to_rgb.b = final_y - 1.1070 * mixed_chroma.x + 1.7046 * mixed_chroma.y;

    float total_opacity = rb_opacity + (afacts * 0.5);
    vec3 result = result_yiq_to_rgb;
    ...
    if(total_opacity > 0.0) {
        ...
        result = mix(result_yiq_to_rgb, pngCol, mask * total_opacity);
    }
