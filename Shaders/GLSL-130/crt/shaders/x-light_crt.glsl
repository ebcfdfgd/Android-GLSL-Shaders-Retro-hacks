...
/* 777-LITE-TURBO-V2-PERFECT-BALANCE
...
uniform sampler2D Texture; uniform vec2 TextureSize, InputSize;
...
void main() {
    ...
    // 2. Sample & Brightness
    vec3 res = texture(Texture, (p_curved + 0.5) / sc).rgb;
    res *= BRIGHT_BOOST;
    ...
    // 5. Balanced RGB Mask (المعالجة الجديدة لمنع زيادة الأحمر)
    float pos = mod(gl_FragCoord.x, floor(MASK_W));
    float W = floor(MASK_W);
    
    // حساب قوة كل لون بناءً على موقعه النسبي بدقة
    vec3 mcol;
