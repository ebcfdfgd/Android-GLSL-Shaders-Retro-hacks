...
/* ARCADE-MASTER-ULTRA-V5
    - PERFORMANCE: Removed Quilez scaling to use hardware filtering (Zero CPU overhead).
    - BRANCHLESS: Replaced Screen Curve "IF" with mathematical clipping.
    - STABLE: Retained all features (Glow, Shift, Mask, Grade) with high speed.
*/
...
void main() {
    // [1] حساب الانحناء بطريقة Branchless
    ...
    // [2] Clipping الرياضي (بديل الـ IF)
    vec2 bounds = step(abs(d_uv), vec2(0.5));
    float check = bounds.x * bounds.y;

    vec2 pos = (d_uv + 0.5) / sc_inv;

    // [3] سحب اللون المباشر (Hardware Accelerated - No Quilez)
    vec3 col = texture2D(Texture, pos).rgb;
    col *= col; 

    // [4] Chromatic Shift & Glow (بقيت كما هي مع تحسين رياضي بسيط)
    if (conv_shift > 0.0 || glow_str > 0.0) {
        vec2 off = (1.2 + conv_shift) / tex_res;
        vec3 col_l = texture2D(Texture, pos - vec2(off.x, 0.0)).rgb;
        vec3 col_r = texture2D(Texture, pos + vec2(off.x, 0.0)).rgb;
    ...
    col *= clamp(1.0 - (r2 * v_amount), 0.0, 1.0);

    // تطبيق الـ Clipping النهائي
