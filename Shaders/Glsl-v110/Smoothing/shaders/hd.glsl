...
/* ULTIMATE SONIC 2026 - NATURAL LIGHTING (SMART BYPASS EDITION)
    - Optimization: Full Bypass when parameters are zero (Saves Battery).
    - Brightness: Native game levels with clean output.
    - Performance: Extreme Speed with branchless logic.
*/
...
uniform float CHROMA_STR, LENS_DIST, NTSC_STR, EDGE_SHINE, OUTLINE_STR, MICRO_AO, AO_SKIN_PROT, RIM_LIGHT, BLOOM_GLOW, VIBRANCE, WARMTH, BLACK_DEPTH;
#endif
...
void main() {
    ...
    // سحب البكسل المرجعي أولاً لاختبار الحالة
    vec3 col_m = texture2D(Texture, texCoord).rgb;

    // بوابة الهروب: لو المزايا الأساسية صفر، اخرج فوراً لتوفير الطاقة والحرارة
    if (CHROMA_STR <= 0.0 && NTSC_STR <= 0.0 && EDGE_SHINE <= 0.0 && MICRO_AO <= 0.0 && BLOOM_GLOW <= 0.0) {
    ...
    // [1] FULL FETCHING (يعمل فقط في حالة تفعيل المزايا)
