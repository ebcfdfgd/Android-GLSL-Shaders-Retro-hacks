...
// Disabled warm tint by default to prevent bright blue objects from turning red/purple
...
// ===== Color Grading & Tone =====
// Zeroed out split toning defaults to prevent global red shifting
...
void main() {
    ...
    // --- De-Dither Integration (Clean Uniform Blend) ---
    vec3 deDitheredC = mix(c, (c + e) * 0.5, de_dither);
    ...
    vec3 work = vec3(redSample.r, deDitheredC.g, blueSample.b);
    ...
    // --- Film S-Curve Contrast (Luminance-based to preserve original colors / Sonic 1 waterfall) ---
    float lum = luma(res);
    float curvedLum = lum * lum * (3.0 - 2.0 * lum);
    float finalLum = mix(lum, curvedLum, af_filmcurve_strength);
    res = res * (finalLum / max(lum, 0.001));
