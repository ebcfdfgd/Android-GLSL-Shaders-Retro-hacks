...
void main() {
    ...
    // --- SGPT Blend (Dither Cleanup) - Kept as is ---
    vec3 diffL = C - L;
    vec3 diffR = C - R;
    ...
    // --- Merge Dither with Halation (Using all 4 directions: L, R, U, D) ---
    vec3 halo_avg = (L + R + U + D) * 0.25;
    vec3 halo_cleaned = mix(halo_avg, cleaned_c, 0.5 * SGPT_BLEND_LEVEL);
    ...
    // --- Fake AO (Ambient Occlusion) Calculation with Threshold ---
    float y_m = luma(res);
