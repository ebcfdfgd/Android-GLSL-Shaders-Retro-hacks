// --- Blargg Turbo-Lite (Mobile Speed Optimized) + RF Grain + Jailbars ---
...
void main() {
    ...
    if (rb_power <= 0.0 && dot_crawl <= 0.0 && jail_str <= 0.0 && sig_noise <= 0.0 && COL_BLEED <= 0.0 && de_dither <= 0.0) {
    ...
    // تم التعديل: إزالة TextureSize.x لجعل التردد مستقلاً عن الدقة
    ...
    if (COL_BLEED > 0.0) {
        float iL = dot(cL, kI);
        i = mix(i, iL, (0.3 * red_persistence) + 0.2);
    }
