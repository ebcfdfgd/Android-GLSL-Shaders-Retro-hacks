...
uniform float COL_BLEED, rb_power, rb_size, rb_detect, rb_speed, rb_tilt, ntsc_blur;
...
// Fast Triangle Wave (sin/cos replacement)
...
void main() {
    ...
    fI = mix(fI, (bcL.y + bcR.y) * 0.5, 0.7 * has_bleed);
    fQ = mix(fQ, (bcL.z + bcR.z) * 0.5, 0.7 * has_bleed);
