...
void main() {
    ...
    // حساب إحداثيات الشبكة في مرحلة الـ Vertex لتوفير الأداء
...
// دالة توليد ضجيج البلاستيك لمحاكاة سطح الشاشة
float plastic_noise(vec2 uv) {
    return fract(sin(dot(uv, vec2(12.9898, 78.233))) * 43758.5453);
}

void main() {
    // [1] سحب الصورة (عينة واحدة لضمان سرعة الأداء)
    vec3 col = texture2D(Texture, vTexCoord).rgb;

    // [2] معالجة الألوان والـ Gamma
    col = (col - vec3(GBA_BLACK)) * GBA_BRIGHTNESS;
    col = pow(max(col, 0.0), vec3(GBA_GAMMA));
    
    // حساب السطوع لتطبيق الضجيج بشكل ذكي
    float luma = dot(col, vec3(0.3, 0.59, 0.11));
    col = mix(vec3(luma), col, GBA_SAT);

    // [3] إضافة نسيج البلاستيك (Plastic Grain)
    // يظهر التأثير أكثر في المناطق الداكنة لمحاكاة تشتت الضوء على البلاستيك
    col += (plastic_noise(vGridCoord) - 0.5) * GBA_GRAIN * (1.0 - luma);

    // [4] منطق الماسكات المتعددة (Hybrid Engine)
    vec3 mask = vec3(1.0);
    
    // أولاً: حساب الشبكة (Grid) - محاكاة الفواصل بين البكسلات
    // 6.28318 = 2 * PI
    ...
    // ثانياً: حساب النقاط (Dots) مع ميزة تلاشي المركز (Center Fade)
    float dots_raw = sin(vGridCoord.x * 6.28318) * sin(vGridCoord.y * 6.28318);
    dots_raw = clamp(dots_raw * 0.5 + 0.5, 0.0, 1.0);
    
    // تلاشي الماسك في المركز لتقليل إجهاد العين وزيادة الوضوح
    ...
    // [5] اختيار النمط المفعّل (Mode Selection)
    if (MASK_MODE < 0.5) {
        mask = final_grid; // النمط 0: شبكة كلاسيكية
    } else if (MASK_MODE < 1.5) {
        mask = final_dots; // النمط 1: نقاط متوزعة
    } else {
        mask = final_grid * final_dots; // النمط 2: هجين (الأكثر واقعية لـ GBA)
    }

    // [6] دمج النتائج وتطبيق التعزيز النهائي للإضاءة
