// --- NTSC ADAPTIVE-ULTIMATE (Luma Dither Blur + Persistence + RF Grain + Black Level) ---
...
uniform float ntsc_scale, ntsc_hue, ntsc_bright, ntsc_black, dither_blur, red_persistence, ntsc_tilt, ntsc_crawl, merge_fields, custom_art, fring_val, rb_power, fr_str, sat_boost, bleed_str;
...
void main() {
    ...
    // Optimization check
    if (dither_blur <= 0.0 && rb_power <= 0.0 && fr_str <= 0.0 && bleed_str <= 0.0 && red_persistence <= 0.0 && sig_noise <= 0.0) {
        float luma_m = dot(col_m, vec3(0.2989, 0.5870, 0.1140)) * ntsc_bright;
        vec3 final_rgb = mix(vec3(luma_m), col_m * ntsc_bright, sat_boost);
        final_rgb = mix(vec3(ntsc_black), vec3(1.0), final_rgb); // Apply Black Level
