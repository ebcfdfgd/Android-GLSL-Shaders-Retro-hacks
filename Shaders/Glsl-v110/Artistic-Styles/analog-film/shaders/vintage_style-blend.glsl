...
#pragma parameter vt_tint_strength      "Vintage - Color Tint Strength"    0.30 -1.0 1.0  0.02
#pragma parameter vt_tone_balance       "Vintage - Tone Balance"           0.50 0.2 0.8  0.02

// ===== Vintage - Grain =====
#pragma parameter grain_str             "Grain Strength"                   2.5  0.0 20.0 1.0
...
void main() {
    ...
    // ===== Faded Base Tone =====
    vec3 res = clamp((color - vt_black_level) / max(1.0 - vt_black_level, 0.001), 0.0, 1.0);
    res *= vt_brightness;
    res = (res - 0.5) * vt_contrast + 0.5;
    res = clamp(res, 0.0, 1.0);

    float lum = luma(res);
    res = mix(vec3(lum), res, vt_saturation);
    res = clamp(res, 0.0, 1.0);

    // ===== Dynamic Split Tone (Driven by Master Color Hue) =====
    // Light leak uses the direct hue
    vec3 leakColor = hue2rgb(vt_theme_hue);
    
    // Highlights get a slightly shifted hue (+0.05) to blend naturally
    vec3 highlightTint = hue2rgb(fract(vt_theme_hue + 0.05)) * 0.20;
    
    // Shadows get the complementary color (+0.50 on the color wheel)
    vec3 shadowTint = hue2rgb(fract(vt_theme_hue + 0.50)) * 0.12;

    float lum2 = luma(res);
    float shadowMask = 1.0 - smoothstep(vt_tone_balance - 0.2, vt_tone_balance + 0.2, lum2);
    float highlightMask = 1.0 - shadowMask;
    
    res += shadowTint * shadowMask * vt_tint_strength;
    res += highlightTint * highlightMask * vt_tint_strength;
    res = clamp(res, 0.0, 1.0);

    // ===== Light Leak =====
    vec2 frame_scale = TextureSize / InputSize;
    vec2 norm_uv = uv * frame_scale;
    float leakDist = distance(norm_uv, vec2(0.85, 0.15));
    float leakMask = 1.0 - smoothstep(0.0, 0.7, leakDist);
    float leakLuma = luma(res);
    float leakVisibility = smoothstep(vt_leak_threshold - 0.2, vt_leak_threshold + 0.2, 1.0 - leakLuma);
    
    res += leakColor * leakMask * leakVisibility * vt_leak_amount;
    res = clamp(res, 0.0, 1.0);

    // ===== Vignette =====
    res = vtVignette(res, uv);

    // ===== Grain =====
    float grain = filmGrain(uv, grain_str, float(FrameCount));
    res += grain;
