...
void main() {
    ...
    // [1] الـ Ghosting (إصلاح منطق الإزاحة)
    if (GHOST_STR > 0.0) {
    ...
    // [2] مصفوفات الألوان (تم إصلاح نظام الأعمدة Column-Major)
    if (PROFILE > 0.5) {
        if (PROFILE < 1.5)      col = col * mat3(0.84, 0.08, 0.08, 0.16, 0.76, 0.08, 0.00, 0.16, 0.84); // mGBA
        else if (PROFILE < 2.5) col = col * mat3(0.70, 0.15, 0.15, 0.20, 0.70, 0.10, 0.10, 0.15, 0.75); // GBC
        else if (PROFILE < 3.5) { col = col * mat3(0.75, 0.10, 0.15, 0.15, 0.75, 0.20, 0.10, 0.15, 0.65); col.b += 0.05; } // SP
        else if (PROFILE < 4.5) { col = col * mat3(0.85, 0.10, 0.05, 0.10, 0.85, 0.10, 0.05, 0.10, 0.85); col += 0.03; } // GG
    ...
    // [3] المعالجة النهائية (الترتيب الصحيح)
    
    // تشبع الألوان
    col = mix(vec3(dot(col, vec3(0.21, 0.72, 0.07))), col, SATURATION);
    
    // الضجيج (يُطبق بعد تصحيح الألوان)
    float noise = pseudo_noise(uv * TextureSize + time * 0.01);
    col = mix(col, col * (0.9 + 0.2 * noise), LCD_GRAIN);
    
    // مستوى السواد
    col = clamp(col - BLACK_LEVEL, 0.0, 1.0);

    // التنعيم (باستخدام العينات الأصلية لتفادي ضبابية الضجيج)
    if (EDGE_SOFT > 0.0) {
