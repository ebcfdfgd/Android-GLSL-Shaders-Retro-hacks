/* PHYSICAL-SIGNAL-EMULATOR-V21-SMOOTH-GRAIN-RED-SMEAR
   - Optimized for RetroArch Android - GLSL 110
   - ADDED: Red Persistence (Smear) parameter
*/
...
#pragma parameter RED_PERSISTENCE "Red Smear Intensity" 0.0 0.0 2.0 0.1
...
uniform float NTSC_FREQ, NTSC_SENSE, rb_tilt, ntsc_hue, COL_BLEED, RED_PERSISTENCE, de_dither, NTSC_SHARP, CRAWL_SPEED, NTSC_ARTIFACTS, NTSC_SATURATION;
...
void main() {
    ...
    float final_y = mix(yM, (yL + yR) * 0.5, 0.5 * de_dither);
    ...
    // Sharpening
    float sharp = final_y - (yL + yR) * 0.5;
    final_y += clamp(sharp * NTSC_SHARP, -0.1, 0.1); 
    ...
    // --- ADDED: Red Smear Logic ---
    // نستخدم الـ bcL (العينة اليسرى) لنقوم بعمل "Smear" للون الأحمر في اتجاه الحركة
