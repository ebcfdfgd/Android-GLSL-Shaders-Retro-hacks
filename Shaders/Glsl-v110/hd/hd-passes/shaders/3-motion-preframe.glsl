...
uniform sampler2D Prev; // الفريم السابق
...
void main() {
    ...
    vec3 prevCol = texture2D(Prev, uv).rgb; // قراءة الفريم السابق
    
    // حساب الوهج (Bloom) من الصورة الحالية
    vec3 blur = (texture2D(Texture, uv + px).rgb + texture2D(Texture, uv - px).rgb) * 0.5;
    col += max(lum(blur) - SOFT_BLOOM, 0.0) * BLOOM_STR;
    
    // دمج الصورة الحالية مع السابقة بناءً على قيمة الموشن
    col = mix(col, prevCol, MOTION_BLUR * 0.8);
