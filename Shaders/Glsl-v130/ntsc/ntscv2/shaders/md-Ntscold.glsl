...
/* --- NTSC MEGA LITE + CHROMA (v130 Sharp Build) ---
   - Optimized: Android Mobile (Vertex Hue Trigs)
   - Features: Resolution, Sharpness, Fringing, Artifacts
   - Performance: Exactly 5 Texture Fetches
*/
...
void main() {
    // 0. استخدام ntsc_res للتحكم في دقة الخط الأفقي
    ...
    // دمج Artifacts لزيادة قوة الرينبو في مناطق التفاصيل
    float total_rb = rb_power + (afacts * 0.5);
    float rainbowI = sin(angle) * total_rb * rb_mask;
    float rainbowQ = cos(angle) * total_rb * rb_mask;

    // --- 5. FINAL LUMA (Sharpness Boost) ---
    float y = yiq.r;
    y += (yiq.r - lumaL) * (ntsc_sharp * 0.6); // تعزيز حدة الحواف
    ...
    // إضافة تأثير Fringing (تسرب الألوان عند الحواف)
