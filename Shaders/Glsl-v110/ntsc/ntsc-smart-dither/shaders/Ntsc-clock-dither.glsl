...
/* NTSC-HYBRID ULTIMATE (1001 Logic + Chroma Bleed Fix)
   - Feature: Smart Adaptive Blur for Dither patterns.
   - Fix: Independent Chroma Fetching for clean color bleed.
   - Performance: Thermal-optimized Bypass for Android.
*/
...
void main() {
    ...
    if (ARTF_STR <= 0.0 && RAINBOW_STR <= 0.0 && CHROMA_BLEED <= 0.0 && red_persistence <= 0.0 && SHARPNESS <= 0.0 && ntsc_blur <= 0.0) {
    ...
    // Apply Blur for Dither patterns
    vec3 col = col_center;
    if (ntsc_blur > 0.0) {
    ...
    // Artifacting (Fringing & Rainbow tied to d_mask for realism)
