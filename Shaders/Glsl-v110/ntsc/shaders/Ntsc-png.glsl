...
// --- NTSC MEGA LITE (ULTRA-GRAIN MERGE - CLEANED) ---
...
#pragma parameter NOISE_STR "Analog Signal Noise" 0.04 0.0 0.5 0.01
...
uniform float SATURATION, BLACK_LEVEL, de_dither, rb_opacity, rb_zoom, rb_speed, rb_tilt, rb_sat, red_persistence, NOISE_STR, COL_BLEED, horiz_sharp;
uniform float CRAWL_SPEED;
#endif
...
// High-precision Hash (Ultra-Grain Logic)
float hash(vec2 co) {
...
void main() {
    ...
    float resI = (cur_i * vHueTrig.y - cur_q * vHueTrig.x) * SATURATION;
    float resQ = (cur_i * vHueTrig.x + cur_q * vHueTrig.y) * SATURATION;
    ...
    // High-Precision RF Grain (Ultra Logic)
    if(NOISE_STR > 0.0) {
        final_y += (hash(uv + time * 0.01) - 0.5) * NOISE_STR;
    }
    ...
    // 7. Apply Black Level
    result = mix(vec3(BLACK_LEVEL), vec3(1.0), result);
