// --- NTSC-1000-RGB-PRO-V3 (Crawl + RF Grain) ---
...
uniform float ntsc_hue, SATURATION, BLACK_LEVEL, rb_power, rb_speed, de_dither, COL_BLEED, pi_mod, vert_scal;
...
void main() {
    ...
    // --- Smart Bypass System ---
    if (rb_power <= 0.0 && sig_noise <= 0.0 && COL_BLEED <= 0.0 && de_dither <= 0.0 && SATURATION == 1.0 && BLACK_LEVEL == 0.0) {
    ...
    // 7. FINAL ASSEMBLY (Applying Hue & Saturation)
