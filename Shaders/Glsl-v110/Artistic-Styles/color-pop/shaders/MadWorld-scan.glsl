...
#pragma parameter MW_EDGE_SENS "Edge Detection Sensitivity" 1.0 0.1 5.0 0.1
...
// ============================================================
// Optimized 4-Neighbor Edge Detection
// 4 fetches here + 1 in main = 5 total
// ============================================================
float detectSobelEdge(
    sampler2D tex,
    vec2 coords,
    vec2 px
) {
...
void main() {
    ...
    // 2. Raw Edge Calculation with Sensitivity
    float rawEdge =
        detectSobelEdge(
            Texture,
            uv,
            px
        ) * MW_OUTLINE_STR * MW_EDGE_SENS;

    // 3. White Details Protection
    float whiteMask =
        smoothstep(
            MW_WHITE_PROTECT - 0.1,
            MW_WHITE_PROTECT + 0.05,
            luma
        );
