...
/* 777-V8-ULTIMATE-5050-QUILEZ
    - 5050-EXACT: Quilez scaling mapping logic retained.
    - LOTTES-GAUSSIAN: Physical beam fading (Turbo-Optimized).
    - ZERO-BRANCHING: Replaced all Logic-Gaps with pure math for GPU speed.
*/
...
void main() {
    ...
    // [B] QUILEZ SCALING (مطابقة لـ 5050 بالضبط)
    ...
    res *= res; // Linear Space لضمان دقة Lottes

    // [C] Lottes Dynamic Scanlines (Turbo Gaussian Mix)
    float dst = fract(tex_uv.y * TextureSize.y * SCAN_DENS) - 0.5;
    float scan = exp2(hardScan * dst * dst);
    res = mix(res, res * scan, SCAN_STR);

    // [D] Optimized Mask System (Branchless Selection)
