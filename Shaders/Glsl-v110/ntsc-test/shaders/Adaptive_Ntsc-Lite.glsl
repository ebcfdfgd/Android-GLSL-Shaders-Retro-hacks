// --- NTSC ADAPTIVE-ULTIMATE (Luma Dither Blur + Persistence + Noise + Jailbars) ---
...
uniform float ntsc_scale, ntsc_hue, ntsc_bright, dither_blur, red_persistence, ntsc_tilt, ntsc_crawl, merge_fields, custom_art, fring_val, rb_power, fr_str, sat_boost, bleed_str;
...
void main() {
    ...
    if (dither_blur <= 0.0 && rb_power <= 0.0 && fr_str <= 0.0 && bleed_str <= 0.0 && red_persistence <= 0.0 && jail_str <= 0.0 && NOISE_STR <= 0.0) {
    ...
    // --- إضافات: Jailbars & Noise ---
    if (jail_str > 0.0)
        final_y += sin(vTexCoord.x * TextureSize.x * jail_width) * jail_str * 0.02;
    
    if (NOISE_STR > 0.0)
        final_y += (fast_rand(vTexCoord + time * 0.01) - 0.5) * NOISE_STR;
    ...
    // 6. CHROMA BLEED & RED PERSISTENCE (Logic 1010)
