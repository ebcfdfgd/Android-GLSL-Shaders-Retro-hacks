/* --- 777-NTSC-HYBRID-ULTIMATE-V3.2 ---
    - ADDED: NTSC Hue control (Color Rotation).
    - ADDED: NTSC Brightness & Black Level control.
    - FIXED: Precision issue (highp) for smooth gradients.
...
uniform float NTSC_BRIGHT, BLACK_LEVEL, ntsc_hue, CC_FREQ, NTSC_TILT, JITTER_STR, ARTF_STR, RAINBOW_STR, CHROMA_BLEED, SHARPNESS, SATURATION, ntsc_blur, sig_noise;
#endif
...
void main() {
    ...
    // --- 1. SMART BYPASS (Optimized) ---
    if (ARTF_STR <= 0.0 && RAINBOW_STR <= 0.0 && CHROMA_BLEED <= 0.0 && SHARPNESS <= 0.0 && ntsc_blur <= 0.0 && sig_noise <= 0.0 && BLACK_LEVEL == 0.0 && NTSC_BRIGHT == 1.0 && ntsc_hue == 0.0) {
    ...
    // Hue Adjustment Logic
    if (ntsc_hue != 0.0) {
        float h_sin = sin(ntsc_hue);
        float h_cos = cos(ntsc_hue);
        float i_new = i * h_cos - q * h_sin;
        float q_new = i * h_sin + q * h_cos;
        i = i_new;
        q = q_new;
    }
