...
/* GBA-REALITY-REMNANT (Final Revision + Black Level)
   - Parameters with GBA_ prefix.
   - Original 5-tap structure maintained for maximum accuracy.
*/
...
void main() {
    ...
    // [1] الـ Ghosting مع الـ Jitter (سحبتين إضافيتين)
    if (GBA_GHOST > 0.0) {
    ...
    col = mix(col, col * (0.9 + 0.2 * noise), GBA_GRAIN);
    
    // [4] التصحيح اللوني وتطبيق Black Level
    // نطبق الـ Black Level هنا قبل التباين والسطوع
    ...
    // [5] التنعيم النهائي (Edge Softening - سحبتين إضافيتين)
    vec3 sN = (texture(Texture, uv - ps * vec2(1.0, 0.0)).rgb + texture(Texture, uv + ps * vec2(1.0, 0.0)).rgb) * 0.5;
