...
/* 777-NTSC-MEGA-LITE-CLEAN (4-TAP DITHER OPTIMIZED)
    - FIXED: 4-Tap dither removal (Up, Down, Left, Right).
...
void main() {
    ...
    vec2 ps_y = vec2(0.0, 1.0 / TextureSize.y);
    ...
    vec3 cU = texture2D(Texture, vTexCoord + ps_y * d_off).rgb;
    vec3 cD = texture2D(Texture, vTexCoord - ps_y * d_off).rgb;

    vec3 yiqM = RGB_to_YIQ * cM;
    float yL = (RGB_to_YIQ * cL).x;
    float yR = (RGB_to_YIQ * cR).x;
    float yU = (RGB_to_YIQ * cU).x;
    float yD = (RGB_to_YIQ * cD).x;

    // 4-Tap Smoothing
    float final_y = (de_dither > 0.0) ? mix(yiqM.x, (yL + yR + yU + yD) * 0.25, 0.5 * de_dither) : yiqM.x;
    ...
    if (rb_power > 0.0) {
        float edge = abs(yiqM.x - yL) + abs(yiqM.x - yR);
