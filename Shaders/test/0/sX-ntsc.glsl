/* 777-CRT-ANALOG-MASTER (STRICT 4-TAP HARDWARE EDITION)
    - FIXED: Dither Mask fully rebuilt using the 1-pixel immediate right tap to capture and melt vertical waterfall stripes.
    - ULTRA-OPTIMIZATION: Strictly locked to 4 HARD TEXTURE TAPS by clever pixel-boundary reuse.
    - REMARK: Rainbow Intensity (RAINBOW_STR) remains completely decoupled from sensitivity scaling.
    - CLEANED: Zero noise engines or extra arithmetic stutter factors.
*/
...
#else
#define CHROMA_BLEED_X 4.0
#define BLUR_TAPS 1.0
#define NTSC_SAT 1.0
#define BRIGHTNESS 1.1
#define RAINBOW_STR 0.35
#define RAINBOW_SIZE 4.5
#define RAINBOW_TILT 0.0
#define RAINBOW_SENS 0.3
#define CRAWL_SPEED 0.02
#endif
...
void main() {
    ...
    // جعل عينة الـ Blur ثابتة على مسافة بكسل واحد دائمًا لضمان صهر تردد الديذر والشلال بدقة 100%
    ...
    vec3 blurRGB        = texture2D(Texture, coord + blurX).rgb;    // Tap 2 (Right Neighbour / Dither Blur)
    vec3 chromaLeftRGB  = texture2D(Texture, coord - bleedX).rgb;   // Tap 3 (Wide Left Chroma)
    vec3 chromaRightRGB = texture2D(Texture, coord + bleedX).rgb;   // Tap 4 (Wide Right Chroma)

    // --- SMART ADAPTIVE DITHER MASK ---
    float y_m = dot(baseRGB,       lumaWeight);
    float y_l = dot(chromaLeftRGB, lumaWeight);
    float y_r = dot(blurRGB,       lumaWeight);

    // حساب التناوب الحاد عالي التردد بين البكسل الحالي وجاره الأيمن الملاصق مباشرة
    ...
    // صهر خطوط الشلال العمودية بطريقة ذكية عن طريق خلط البكسل الحالي مع جاره لإنتاج الشفافية الكاملة
    vec3 ditheredRGB = mix(baseRGB, mix(blurRGB, chromaLeftRGB, 0.5), 0.5 * dither_mask);
    ...
    // 1. بوابة التصفية وطرح العتبة
    float edgeMask = clamp((diffH * RAINBOW_SENS) - 0.05, 0.0, 1.0);
    
    // 2. تعزيز الحواف الحادة حسابياً وبسرعة فائقة
