...
/* TIMOTHY LOTTES - HYBRID V5.7 (SCREEN-SPACE SCANLINES)
    - UPDATED: Scanlines aligned to Screen Resolution (gl_FragCoord)
    - ADDED: Scanline Cycle/Density Parameter
*/
...
#pragma parameter scan_res "Scanline Cycle (Pixels)" 3.0 2.0 10.0 1.0
...
uniform float BARREL_DISTORTION, VIG_STR, hardScan, scan_str, scan_res, hardPix, maskDark, maskLight, shadowMask, brightBoost, bloomAmount;
#endif
...
void main() {
    ...
    // 4. SCREEN-SPACE SCANLINES
    if (scan_str > 0.0) {
        float lum = dot(color, vec3(0.299, 0.587, 0.114));
        // نستخدم gl_FragCoord.y عشان نربط الخطوط ببيكسل الشاشة
        float dst = fract(gl_FragCoord.y / scan_res) - 0.5;
