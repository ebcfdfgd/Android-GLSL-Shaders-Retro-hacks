...
/* ULTIMATE-HYBRID (300 Engine - Backported to 110)
   ...
   - OPTIMIZATION: 5050-Texture-Flat Sampling logic.
   - THERMAL: Smart Branching to save GPU power.
*/
...
void main() {
    ...
    // [2] معادلة الشرب السريع (Quilez Scaling - 5050 Method)
    vec2 fetch_uv = (d_uv + 0.5) / screen_scale;
    vec2 p = fetch_uv * TextureSize;
    vec2 i = floor(p);
    vec2 f = p - i;
    f = f * f * (3.0 - 2.0 * f); // S-Curve لمنع التموج
    
    // سحب الصورة الأساسية بدقة 5050
    vec3 res = texture2D(Texture, (i + f + 0.5) * inv_tex_size).rgb * BRIGHT_BOOST;
