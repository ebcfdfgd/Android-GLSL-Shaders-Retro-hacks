// --- NTSC ADAPTIVE-ULTIMATE (113 Logic + Noise & Jailbars) ---
#version 110
...
#pragma parameter noise_str "Analog Signal Noise" 0.05 0.0 0.5 0.01
...
void main() {
    ...
    if (ntsc_blur <= 0.0 && rb_power <= 0.0 && fr_str <= 0.0 && bleed_str <= 0.0 && red_persistence <= 0.0 && noise_str <= 0.0 && jail_str <= 0.0) {
    ...
    // إضافة ARTIFACTS (Jailbars & Noise)
    if (jail_str > 0.0) final_y += sin(vTexCoord.x * TextureSize.x * jail_width) * jail_str * 0.02;
    if (noise_str > 0.0) final_y += (rand(vTexCoord + vec2(mod(time, 100.0) * 0.01, 0.0)) - 0.5) * noise_str;
