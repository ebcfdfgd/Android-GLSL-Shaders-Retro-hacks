...
/* GBA-ULTIMATE-HYBRID-PRO (V3.1 - Fixed 103 Radial Vignette)
    - FIXED: Vignette centering and aspect ratio scaling.
...
void main() {
    ...
    // Core Sampling
    vec3 col_center = texture(Texture, TEX0).rgb;
    float r_sample = texture(Texture, TEX0 - ps * (soft + chroma + ghost_osc) + striping).r;
    float b_sample = texture(Texture, TEX0 + ps * (soft + chroma) + striping).b;
    float g_sample = (texture(Texture, TEX0 - ps * soft).g + texture(Texture, TEX0 + ps * soft).g) * 0.5;
    
    vec3 res = mix(col_center, vec3(r_sample, g_sample, b_sample), ANALOG_SOFT);
    vec3 col_ghost = texture(Texture, TEX0 - ps * (GBA_GHOST * 0.5 + ghost_osc)).rgb;
    res = mix(res, col_ghost, GBA_GHOST * 0.4);

    // Color Profiles
    vec3 target_mat;
    if (COLOR_MODE < 0.5) target_mat = res;
    else if (COLOR_MODE < 1.5) target_mat = vec3(res.r * 0.82 + res.g * 0.18, mix(res.g * 0.70 + res.b * 0.30, res.g, GREEN_BAL), res.r * 0.05 + res.b * 0.95);
    else if (COLOR_MODE < 2.5) target_mat = vec3(res.r * 0.70 + res.g * 0.30, res.r * 0.10 + res.g * 0.80 + res.b * 0.10, res.r * 0.05 + res.g * 0.15 + res.b * 0.80);
    else if (COLOR_MODE < 3.5) {
        float gb_luma = dot(res, vec3(0.3, 0.59, 0.11));
        target_mat = mix(vec3(0.1, 0.15, 0.05), vec3(0.6, 0.7, 0.1), gb_luma);
    } else target_mat = vec3(res.r * 0.95 + res.g * 0.05, res.r * 0.02 + res.g * 0.95 + res.b * 0.03, res.r * 0.05 + res.b * 0.95);
    ...
    // --- [Fixed 103 Radial Vignette] ---
    // تصحيح المركز ليكون في منتصف الشاشة تماماً بغض النظر عن الأبعاد
