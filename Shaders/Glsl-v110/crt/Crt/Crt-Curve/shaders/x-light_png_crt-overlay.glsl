...
/* ULTIMATE-TURBO-HYBRID (V11-SCAN-OVERLAY)
    - SCANLINES: Smart Overlay (Hybrid Multiply/Screen logic).
    - MASK: High-performance PNG texture support.
    - OPTIMIZED: Clean math for mobile GPUs.
*/
...
void main() {
    ...
    // [D] Smart Scanlines (Applied with Overlay Logic)
    if (SCAN_STR > 0.0) {
        float scan = abs(fract(gl_FragCoord.y * (1.0 / SCAN_DENS) - 0.5) - 0.5) * 4.0;
        scan = clamp(scan, 0.0, 1.0);
        
        // معادلة الأوفلاي الهجينة للسكان لاينز فقط
        // إذا كان السطوع أقل من 0.8: تدمج بأسلوب الضرب (Multiply) لعمق الظلال
        // إذا كان السطوع أعلى من 0.8: تدمج بأسلوب الشاشة (Screen) لحماية المناطق الساطعة
        vec3 scan_ovl = (res < vec3(0.8)) ? (res * scan) : (1.0 - (1.0 - res) * (1.0 - (scan - 1.0)));
        res = mix(res, scan_ovl, SCAN_STR);
    }

    // [E] PNG-Only Mask System (Standard Mix)
    if (MASK_STR > 0.0) {
