...
/* 777-ULTRA-PURE-SCAN-V14-LOTTES-HYBRID
    - INTEGRATED: Ultra-Fast Lottes Scanlines (exp2 method).
...
uniform float LCD_STR, LCD_SIZE, hardScan, SCAN_STR, BRIGHTNESS_LCD;
#endif
...
void main() {
    ...
    // Dynamic strength modifier: fades to 0.0 as color approaches pure white
    float white_fade = clamp((1.0 - luma) * 4.0, 0.0, 1.0); 
    float current_scan_str = SCAN_STR * white_fade;

    // 3. LOTTES SCANLINES (Pixel-Perfect with Game Resolution)
    float dst = fract(vTexCoord.y * TextureSize.y) - 0.5;
