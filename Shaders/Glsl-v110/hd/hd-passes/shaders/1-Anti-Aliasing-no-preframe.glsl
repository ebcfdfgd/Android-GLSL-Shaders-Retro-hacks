...
#pragma parameter JITTER_STRENGTH "Spatial Jitter Reducer" 0.50 0.00 1.00 0.05
...
void main() {
    ...
    // 4. تثبيت الاهتزاز المكاني (Spatial Jitter Reduction)
    // بدلاً من استخدام Prev، نقوم بدمج البكسل الحالي مع محيطه 
    // لتقليل حدة "الرعشة" الناتجة عن البكسلات المتذبذبة
    vec3 spatialBlur = (col + L + R + U + D) * 0.2;
    vec3 finalCol = mix(aa, spatialBlur, JITTER_STRENGTH * 0.5);
