...
void main() {
    ...
    // [2] AO CALCULATION (Cavity Detection)
    // نقارن المركز بمتوسط سطوع الجيران (اليمين والأعلى)
    float avgNeighbor = (lr + lu) * 0.5;
    float ao = clamp((avgNeighbor - lc) * AO_STR, 0.0, 1.0);
    vec3 col = C * (1.0 - ao); // تطبيق الـ AO (تغميق التجويف)
    
    // [3] EDGE & OUTLINE CALCULATION
    float edge = abs(lr - lc) + abs(lu - lc);
    
    // [4] APPLY EFFECTS
    col = mix(col, vec3(0.0), smoothstep(0.1, 0.35, edge) * OUTLINE_STR);
    col += smoothstep(0.1, 0.45, edge) * RIM_STR * 0.2;
    
    // [5] TILTSHIFT (باستخدام المتاح)
    col = mix(col, (R + U) * 0.5, smoothstep(0.0, 0.45, abs(uv.y - 0.5)) * TILTSHIFT_STR);
