...
const vec3 AO_TINT = vec3(0.4, 0.3, 0.7);
const vec3 RIM_TINT = vec3(0.8, 0.9, 1.0);
...
void main() {
    ...
    // AO with TNT
    res -= (dist * MICRO_AO * AO_TINT * clamp(1.0 - y_m, 0.0, 1.0)) * step(y_m, AO_SKIN_PROT);
    
    // Rim with TNT
    res += (edge * RIM_LIGHT * 0.7 * rimMask * RIM_TINT);
