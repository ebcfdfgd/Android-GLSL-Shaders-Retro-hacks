...
// --- NTSC MEGA LITE (TURBO OPTIMIZED + FIXED PRECISION) ---
...
uniform float ntsc_blur, rb_opacity, rb_zoom, rb_speed, rb_tilt, rb_sat, red_persistence, NOISE_STR, JAIL_STR, JAIL_WIDTH, tv_mist, COL_BLEED, horiz_sharp;
...
// Hash عالي الدقة لمنع الخطوط (Banding)
float rand(vec2 co){ 
...
void main() {
    ...
    if(COL_BLEED > 0.0) {
        ...
        cur_i = mix(cur_i, (dot(cL, i_v) + dot(cR, i_v)) * 0.5, 0.7);
        cur_q = mix(cur_q, (dot(cL, q_v) + dot(cR, q_v)) * 0.5, 0.7);
    ...
    if(JAIL_STR > 0.0) final_y += sin(uv.x * JAIL_WIDTH * 500.0) * JAIL_STR * 0.05;
    ...
    if(rb_opacity > 0.0) {
        ...
        pUV.x += (time * rb_speed * 0.02) + (time * CRAWL_SPEED * 0.05);
        pUV.y += (time * 0.005); 
