/*
    NTSC-ULTIMATE (Hue & Fidelity Build - Backported to 110)
    - Feature: Integrated Resolution, Fringing, and Hue Control.
    - Feature: Integrated Analog Noise & Vertical Jailbars.
...
#pragma parameter analog_noise "Analog: Signal Noise" 0.05 0.0 0.5 0.01
...
uniform float ntsc_res, ntsc_hue, ntsc_sharp, fring, afacts, JAIL_STR, JAIL_WIDTH, analog_noise;
...
void main()
{
    ...
    vec4 art_b = texture2D(TextureSize.y > 0.0 ? NTSCArtifactSampler2 : NTSCArtifactSampler2, scanuv + vec2(0.0, invSize.y)); // Safety check
    ...
    // [5] Analog Effects (Noise & Jailbars)
    if (analog_noise > 0.0) {
        res += (rand(TEX0 + float(FrameCount) * 0.01) - 0.5) * analog_noise;
    }
    if (JAIL_STR > 0.0) {
        res += sin(TEX0.x * TextureSize.x * JAIL_WIDTH) * JAIL_STR * 0.05;
    }
