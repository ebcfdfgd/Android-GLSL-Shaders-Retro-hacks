// --- NTSC ADAPTIVE-ULTIMATE (Luma Dither Blur + Persistence + RF Grain + Jailbars) ---
...
void main() {
    ...
    if (dither_blur <= 0.0 && rb_power <= 0.0 && fr_str <= 0.0 && bleed_str <= 0.0 && red_persistence <= 0.0 && jail_str <= 0.0 && sig_noise <= 0.0) {
    ...
    // --- إضافات: Jailbars & RF Grain (معدلة) ---
    if (jail_str > 0.0)
