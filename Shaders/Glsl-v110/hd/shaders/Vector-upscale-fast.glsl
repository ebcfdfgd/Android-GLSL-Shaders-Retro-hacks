...
#pragma parameter CORNER_THRESHOLD "Corner Match Tolerance"  1.0 0.0 2.0 0.05
#pragma parameter EDGE_SOFTNESS    "Edge AA Softness"        1.0 0.01 2.0 0.05
...
void main()
{
    ...
    // Replaced smoothstep with linear clamp interpolation
    float simAB = 1.0 - clamp(distance(A, B) / CORNER_THRESHOLD, 0.0, 1.0);
    float simAD = 1.0 - clamp(distance(A, D) / CORNER_THRESHOLD, 0.0, 1.0);
    vec3  ABavg = 0.5 * (A + B);
    float simCenter = 1.0 - clamp(distance(ABavg, C) / CORNER_THRESHOLD, 0.0, 1.0);
    ...
    // Replaced smoothstep with linear clamp interpolation
    float geomBlend = clamp((t + EDGE_SOFTNESS) / (2.0 * EDGE_SOFTNESS), 0.0, 1.0);
