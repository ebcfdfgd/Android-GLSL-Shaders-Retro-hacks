...
uniform float overlay_str2, png_width2, png_height2;
#endif
...
void main() {
    ...
    if (overlay_str > 0.01) {
        vec2 maskUV1 = vec2(fract(mP.x * OutputSize.x / (png_width)), 
                            fract(mP.y * OutputSize.y / (png_height)));
    ...
    if (overlay_str2 > 0.01) {
        vec2 maskUV2 = vec2(fract(mP.x * OutputSize.x / (png_width2)), 
                            fract(mP.y * OutputSize.y / (png_height2)));
