...
/* NTSC-V1-TURBO (Extreme Performance Edition)
    - STRIPPED: Branching logic (no IFs) for maximum GPU throughput.
    - FEATURES: 100% Retained (Mist, Persistence, Rainbow, Noise).
    - OPTIMIZED: Vectorized math for GLES 2.0 / Mali / Adreno.
*/
...
void main() {
    ...
    // تطبيق De-Dither و Mist رياضياً بدون IF
    ...
    // 2. CHROMA ENGINE (Optimized Zero-Branch Bleed)
    float fI = dot(cM, kI);
    float fQ = dot(cM, kQ);
    ...
    // دمج الـ Bleed والـ Persistence في عملية واحدة
