...
#ifdef GL_ES
// السر: استخدام highp لتجنب تكسر النمش (Banding)
...
void main() {
    ...
    // --- 1. SMART BYPASS ---
    if (ARTF_STR <= 0.0 && RAINBOW_STR <= 0.0 && CHROMA_BLEED <= 0.0 && red_persistence <= 0.0 && SHARPNESS <= 0.0 && ntsc_blur <= 0.0 && sig_noise <= 0.0 && jail_str <= 0.0) {
    ...
    // إضافات الأرتيفاكت (Floating Noise) مع Jailbars مستقلة الدقة
    if (jail_str > 0.0) y += sin(pos.x * jail_width * 500.0) * jail_str * 0.02;
    // النمش هنا أصبح ناعماً (Time * 0.01) بدون تكرار
