/* NTSC-1000-RGB-PRO-V3 (Crawl + RF Grain + Jailbars)
   - Optimized: Smart Bypass (Zero-parameter shutdown).
   - Added: Signal RF Grain (Exact Copy).
...
void main() {
    ...
    if (rb_power <= 0.0 && jail_str <= 0.0 && sig_noise <= 0.0 && COL_BLEED <= 0.0 && de_dither <= 0.0) {
    ...
    // 2. EDGE DETECTION & DITHER
    float edge = abs(y_l - y_m) + abs(y_r - y_m) - abs(y_l - y_r);
    float rb_mask = smoothstep(0.02, 0.20, edge);
    float final_y = (de_dither > 0.0) ? mix(y_m, (y_l + y_m + y_r) * 0.3333, de_dither * rb_mask) : y_m;

    // 3. ARTIFACTS: JAILBARS & RF GRAIN (Resolution Independent)
    if (jail_str > 0.0) final_y += sin(vTexCoord.x * jail_width * 500.0) * jail_str * 0.02;
    if (sig_noise > 0.0) final_y += (hash(vTexCoord + time * 0.01) - 0.5) * sig_noise;

    // 4. RGB RAINBOW PHASE ENGINE
    ...
    if (COL_BLEED > 0.0) {
        ...
        if (red_persistence > 0.0) i = mix(i, i_l, 0.6 * red_persistence);
    }
