// --- NTSC ADAPTIVE-ULTIMATE (113 Logic + RF Grain Build + Black Level) ---
#version 110
...
#pragma parameter ntsc_black "NTSC Black Level" 0.0 0.0 0.3 0.01
#pragma parameter ntsc_blur "NTSC Dither Blur" 0.7 0.0 1.0 0.05
#pragma parameter red_persistence "Red Persistence (Right Smear)" 1.5 0.0 2.5 0.05
...
uniform float ntsc_scale, ntsc_hue, ntsc_bright, ntsc_black, ntsc_blur, red_persistence, ntsc_tilt, ntsc_crawl, merge_fields, custom_art, fring_val, rb_power, fr_str, sat_boost, bleed_str, sig_noise;
#else
...
void main() {
    ...
    if (sig_noise > 0.0) {
        final_y += (hash(vTexCoord + vec2(mod(time, 100.0) * 0.01)) - 0.5) * sig_noise;
    }
