...
/* ULTIMATE-TURBO-300 (Performance Build - 5050 DNA)
   ...
   - PERFORMANCE: Early Exit & Smart Branching for Adreno/Mali GPUs.
*/
...
void main() {
    ...
    vec2 i = floor(p);
    vec2 f = p - i;
    f = f * f * (3.0 - 2.0 * f); // تنعيم البكسلات ومنع التموج
    
    // سحب الصورة الأساسية بطريقة 5050
    vec3 gm = texture2D(Texture, (i + f + 0.5) * inv_tex_size).xyz * BRIGHT_BOOST;
    ...
    // 4. نظام التفرع الذكي للطبقة الأولى
    if (OverlayMix > 0.01) {
