/* ULTIMATE SONIC 2026 - OPTIMIZED 4-TAP DITHER ELIMINATION */

#pragma parameter dither_str "Dither Removal Strength" 1.0 0.0 1.0 0.05
...
#pragma parameter RIM_LIGHT "Light: Rim Strength" 1.20 0.0 2.5 0.05
...
void main() {
    ...
    // [1] 4-TAP DITHER ELIMINATION
    vec3 c0 = texture2D(Texture, texCoord).rgb;
    vec3 c1 = texture2D(Texture, texCoord + vec2(px.x, 0.0)).rgb;
    vec3 c2 = texture2D(Texture, texCoord + vec2(0.0, px.y)).rgb;
    vec3 c3 = texture2D(Texture, texCoord + vec2(px.x, px.y)).rgb;
    
    vec3 res = mix(c0, (c0 + c1 + c2 + c3) * 0.25, dither_str);
    float y_m = dot(res, lumaWeight);

    // [2] OUTLINE & ENHANCED RIM-LIGHT
    float edge = dot(abs(res - c1) + abs(res - c2), vec3(0.333));
    float outlineMask = smoothstep(0.0, 0.5, edge) * clamp(1.1 - y_m, 0.0, 1.0);
    res /= (1.0 + (outlineMask * OUTLINE_STR * 1.5));

    // تحسين Rim Light: الاعتماد على حدة التباين مع تنعيم القناع ليعطي إيحاء الإضاءة المحيطة
    float rimMask = pow(clamp(1.0 - (y_m * RIM_MASK_POWER), 0.0, 1.0), 2.0); 
    res -= (abs(dot(c1, lumaWeight) - dot(c2, lumaWeight)) * MICRO_AO * clamp(1.0 - y_m, 0.0, 1.0)) * step(y_m, AO_SKIN_PROT);
    res += (edge * RIM_LIGHT * 1.2 * rimMask); 
