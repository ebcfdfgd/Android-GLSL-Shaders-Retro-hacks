...
/* ULTIMATE-TURBO-HYBRID-V6-SUPERSONIC-FIXED
    - FIXED: Stabilized Lottes Scanlines for PS1/Variable Res.
    - UPDATED: Dark/Light Mask Logic.
...
uniform float BARREL_DISTORTION, BRIGHT_BOOST, v_amount, mask_dark, mask_light, MASK_W, hardScan, SCAN_STR;
#endif
...
void main() {
    ...
    // 5. LOTTES SCANLINES (STABILIZED FIX)
    // نستخدم إحداثيات بكسلات اللعبة الصافية لضمان ثبات الخطوط فوق بكسلات الجهاز الأصلي
    float pos_y = tex_uv.y * TextureSize.y;
    float dst = fract(pos_y) - 0.5;
    
    // معادلة Lottes لتوزيع الضوء بشكل فيزيائي ناعم
    float scanline = exp2(hardScan * dst * dst);
    res = mix(res, res * scanline, SCAN_STR);

    // 6. Pure RGB Mask Logic (Dark/Light Mode)
    ...
    // Apply Dark/Light Mask constraints
    res *= clamp(mcol, mask_dark, mask_light);
