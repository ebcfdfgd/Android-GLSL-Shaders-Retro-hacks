...
/* NTSC MEGA LITE - ANALOG NOISE + CORRECTED BLEED
   - Fix: Independent Chroma Bleed logic for visible color smearing.
   - Fix: Accurate YIQ to RGB weights for CRT-like saturation.
   - Performance: Single-pass texture fetching.
*/
...
uniform float COL_BLEED, rb_power, rb_size, rb_detect, rb_speed, rb_tilt, de_dither, analog_noise;
#endif
...
void main() {
    ...
    // 2. CHROMA BLEED (Independent Fetching for visible effect)
    float fI = dot(cM, kI);
    float fQ = dot(cM, kQ);

    if (COL_BLEED > 0.0) {
        // توسيع نطاق السحب بناءً على COL_BLEED لضمان سيلان الألوان
        ...
        // خلط الألوان المسحوبة مع الأصلية لخلق الـ Bleed التناظري
