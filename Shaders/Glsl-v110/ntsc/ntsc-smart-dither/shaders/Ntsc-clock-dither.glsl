/* --- 777-NTSC-HYBRID-ULTIMATE-V3 ---
    - FIXED: Precision issue (Added highp) to eliminate Noise Banding.
    - FIXED: RF Grain (Smooth "Namash" instead of blocky squares).
    - OPTIMIZED: Smart Bypass remains intact.
...
#ifdef GL_ES
// Use highp to avoid noise banding
...
uniform float BLACK_LEVEL, NTSC_BRIGHT, CC_FREQ, NTSC_TILT, JITTER_STR, ARTF_STR, RAINBOW_STR, CHROMA_BLEED, red_persistence, SHARPNESS, SATURATION, ntsc_blur;
...
void main() {
    ...
    // --- 1. SMART BYPASS (Added Black Level check) ---
    if (ARTF_STR <= 0.0 && RAINBOW_STR <= 0.0 && CHROMA_BLEED <= 0.0 && red_persistence <= 0.0 && SHARPNESS <= 0.0 && ntsc_blur <= 0.0 && sig_noise <= 0.0 && BLACK_LEVEL == 0.0) {
    ...
    // Apply Black Level
    rgb = mix(vec3(BLACK_LEVEL), vec3(1.0), clamp(rgb, 0.0, 1.0));
