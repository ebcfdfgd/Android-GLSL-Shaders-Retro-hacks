...
/* 777-CRT-ANALOG-MASTER (RAW SIGNAL + FAST HALATION ONLY)
    - REMOVED: All Bloom parameters and logic.
...
void main() {
    ...
    // 1. Fast Halation Calculation using nearby samples
    vec3 halo_sample = (texture2D(Texture, uv + px).rgb + texture2D(Texture, uv - px).rgb) * 0.5;
