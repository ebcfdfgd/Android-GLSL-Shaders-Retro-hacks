...
void main() {
    // 1. Zoom logic
    vec2 zoomedTEX = (TEX0 - 0.5) / ZOOM + 0.5;

    // 2. محرك الشرب السريع (Quilez Scaling - DNA 5050)
    vec2 p = zoomedTEX * TextureSize;
    vec2 i = floor(p);
    vec2 f = p - i;
    f = f * f * (3.0 - 2.0 * f); 
    ...
    // 3. إحداثيات الماسكات (Screen Space)
