/* --- 777-NTSC-V1-TURBO-ULTRA-GRAIN (SMART DITHER 16 + SAT/BLACK) ---
    ...
    - ADDED: Saturation & Black Level Controls.
*/
...
uniform float SATURATION, BLACK_LEVEL, COL_BLEED, red_persistence, rb_power, rb_size, rb_detect, rb_speed, rb_tilt, ntsc_blur, sig_noise;
...
void main() {
    ...
    fI = mix(fI, ((dot(bcL, kI) + dot(bcR, kI)) * 0.5) * SATURATION, 0.7 * has_bleed);
    fQ = mix(fQ, ((dot(bcL, kQ) + dot(bcR, kQ)) * 0.5) * SATURATION, 0.7 * has_bleed);
    fI = mix(fI, dot(bcL, kI) * SATURATION, 0.4 * red_persistence * step(0.001, red_persistence));
