...
/* ULTIMATE-HYBRID (300 Engine - Backported to 110)
   ...
   - Compatibility: Full support for Mali/Adreno on GLSL 1.10.
*/
...
// منطق دمج الطبقات (Blending Modes)
...
void main() {
    ...
    // إعداد الإحداثيات للتقوس (Toshiba Curve)
    ...
    // قطع الحواف خارج نطاق الصورة (Overscan)
    ...
    vec3 res = texture2D(Texture, fetch_uv).rgb * BRIGHT_BOOST;

    // --- Soft Vignette (تعتيم ناعم عند الحواف) ---
    float vignette = d_uv.x * d_uv.x + d_uv.y * d_uv.y;
    res *= clamp(1.0 - (vignette * v_amount), 0.0, 1.0);

    // --- Smart Branching Layer 1 ---
    // إذا كانت الشدة 0، سيتجاهل كرت الشاشة سحب الـ Texture تماماً لتوفير الطاقة
    if (overlay_str > 0.01) {
