...
/*
    ...
    - 7 Samples Total: extremely fast for low-end mobile GPUs.
    - Features: NTSC Artifacts, Color Tilt, and Animated Signal Crawl.
    - Optimization: Uses existing texture fetches for sharpness logic.
*/
...
// دالة حساب السطوع اليدوية لنسخة 110
...
void main()
{
    vec2 SourceSize = vec2(TextureSize.x, TextureSize.y);
    ...
    vec3 TunedNTSC = NTSCArtifact.rgb * Tuning_Artifacts;
    Cur_Local = clamp(Cur_Local + (((Cur_Left - Cur_Local) + (Cur_Right - Cur_Local)) * TunedNTSC), 0.0, 1.0);

    // 4. الـ Sharpness الذكي (نستخدم العينات المسحوبة مسبقاً لتوفير الأداء)
    float curBrt = Brightness(Cur_Local);
    float NBrtL  = Brightness(Cur_Left);
    float NBrtR  = Brightness(Cur_Right);
    
    // معادلة الحدة: تعزيز الفرق بين البكسل وجيرانه
    float sharp_diff = (curBrt * 2.0 - NBrtL - NBrtR);
    Cur_Local = clamp(Cur_Local + (sharp_diff * Tuning_Sharp * mix(vec3(1.0), NTSCArtifact.rgb, Tuning_Artifacts)), 0.0, 1.0);
