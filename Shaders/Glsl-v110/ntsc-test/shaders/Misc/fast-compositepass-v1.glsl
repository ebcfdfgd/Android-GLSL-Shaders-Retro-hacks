/*
    NTSC Shader - Ultra Optimized (Hue, Sharpness, Noise & Jailbars Build)
    - Integrated: Resolution scaling, Sharpness, Fringing, Artifacts, Hue Shift.
    - Integrated: Analog Signal Noise & Vertical Jailbars.
    - Performance: 7-Sample efficiency + procedural noise/jailbars.
*/
...
void main()
{
    ...
    // 6. Analog Noise & Jailbars Integration
    if (analog_noise > 0.0) {
        float n = rand(vTexCoord, mod(float(FrameCount), 100.0) * 0.1);
        Cur_Local += (n - 0.5) * analog_noise;
    }
    
    if (JAIL_STR > 0.0) {
        float jail = sin(vTexCoord.x * TextureSize.x * JAIL_WIDTH) * JAIL_STR;
        Cur_Local += jail * 0.1;
    }
