...
/* 777-NTSC-MEGA-LITE-CLEAN (CONTRAST-SAFE GRAIN) */
...
#pragma parameter ia_GRAIN_STR "Film Grain Strength" 0.0 0.0 72.0 6.0
...
uniform float NTSC_BRIGHTNESS, SATURATION, COL_BLEED, rb_power, rb_size, rb_detect, rb_speed, rb_tilt, de_dither, ia_GRAIN_STR;
#endif

const mat3 RGB_to_YIQ = mat3(0.299, 0.596, 0.211, 0.587, -0.274, -0.523, 0.114, -0.322, 0.312);
const mat3 YIQ_to_RGB = mat3(1.1600, 0.9091, 1.2222, 1.1090, -0.2473, -1.3518, 0.7204, -0.5882, 2.0814);
...
// Exact replica of hunterk's filmGrain function
vec3 filmGrain(vec2 uv, float strength) {
    float x = (uv.x + 4.0) * (uv.y + 4.0) * ((mod(float(FrameCount), 800.0) + 10.0) * 10.0);
...
void main() {
    ...
    // Apply Contrast-Safe Grain
    // This parabola curve ensures the mask is 0.0 at pure black, 1.0 at mid-gray, and 0.0 at pure white.
    // It completely prevents the grain from washing out blacks or clipping whites.
    vec3 luma_mask = res * (1.0 - res) * 4.0;
    res += film_grain * luma_mask;
