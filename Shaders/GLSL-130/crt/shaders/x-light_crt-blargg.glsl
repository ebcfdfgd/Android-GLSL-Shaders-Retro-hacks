...
/* CRT-ULTIMATE-OMNI-PROJECT (Merged Edition)
...
void main() {
    ...
    res = clamp(res * res, 0.0, 1.0); // Linearize for processing

    // --- 4. Color Profiles (Pro-E) ---
    if (CLU_PROFILE > 1.5 && CLU_PROFILE < 2.5) { res = clamp(res * mat3(0.95, 0.05, 0.0, 0.02, 0.98, 0.0, 0.0, 0.05, 0.95), 0.0, 1.0); } 
    else if (CLU_PROFILE > 2.5) { res = clamp(res * mat3(0.9, 0.1, 0.0, 0.05, 0.9, 0.05, 0.0, 0.1, 1.1), 0.0, 1.0); }

    // --- 5. Final CRT Adjustments (Contrast, Saturation, Black Depth) ---
    ...
    // Scanlines & Mask (X-Light Balanced)
