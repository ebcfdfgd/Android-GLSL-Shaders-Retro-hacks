...
uniform float NTSC_BRIGHT, CC_FREQ, NTSC_TILT, JITTER_STR, ARTF_STR, RAINBOW_STR, CHROMA_BLEED, SHARPNESS, SATURATION;
#endif

void main() {
    vec2 pos = vTexCoord;
    
    // [1] سحب العينة المركزية أولاً (الأساس)
    vec3 col_center = texture2D(Texture, pos).rgb;

    // --- شرط الإيقاف الذكي (Bypass) ---
    // إذا كانت قيم التأثيرات صفر، يتم إيقاف سحب العينات الجانبية تماماً وتوفير الأداء
    if (ARTF_STR <= 0.0 && RAINBOW_STR <= 0.0 && CHROMA_BLEED <= 0.0 && SHARPNESS <= 0.0) {
    ...
    // --- 2. المرحلة (Phase) مع التحكم في الميلان ---
    ...
    // 3. سحب عينات الجوار (يتم تفعيلها فقط إذا لم يتحقق شرط الإيقاف)
    ...
    float i_bleed = (dot(bleed_l, vec3(0.5959, -0.2744, -0.3216)) + dot(bleed_r, vec3(0.5959, -0.2744, -0.3216))) * 0.5;
