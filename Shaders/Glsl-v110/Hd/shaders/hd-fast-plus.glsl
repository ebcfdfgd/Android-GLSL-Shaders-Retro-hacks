...
#pragma parameter OUTLINE_STR "Detail: Outline Power" 0.20 0.0 2.0 0.05
#pragma parameter MICRO_AO "Depth: Micro-AO" 0.35 0.0 2.0 0.05
#pragma parameter AO_SKIN_PROT "Depth: AO Skin Protect" 0.60 0.0 1.0 0.05
#pragma parameter RIM_LIGHT "Light: Rim Strength" 0.65 0.0 3.0 0.05
#pragma parameter RIM_MASK_POWER "Rim Mask Threshold" 1.5 0.0 3.0 0.05
#pragma parameter LUMA_ADAPT "Engine: Luma Adaptation" 0.2 0.0 0.5 0.05
#pragma parameter HIGH_COMP "Engine: Highlight Comp" 0.3 0.0 1.0 0.05

// --- COLOR CORRECTION PARAMETERS ---
#pragma parameter CLM_RED "Color: Calm Red Power" 0.25 0.0 1.0 0.05
#pragma parameter GRN_YEL "Color: Green -> Yellowish" 0.15 0.0 1.0 0.05
#pragma parameter BLU_CYN "Color: Blue -> Crisp Cyan" 0.30 0.0 1.0 0.05

#pragma parameter VIBRANCE "Color: Vibrance" 1.40 -1.0 2.0 0.05
#pragma parameter WARMTH "Color: Warmth" 0.2 -1.0 1.0 0.05
...
uniform float dither_str, C_WHT_LVL,C_BLK_LVL, OUTLINE_STR, MICRO_AO, AO_SKIN_PROT, RIM_LIGHT, RIM_MASK_POWER, VIBRANCE, WARMTH, HIGH_COMP, LUMA_ADAPT;
uniform float CLM_RED, GRN_YEL, BLU_CYN;
...
res = res * (1.0 / max(0.001, C_WHT_LVL));
    ...
    // --- SMART COLOR CORRECTION SYSTEM ---
    // 1. Green to Yellowish Shift
    res.r += max(0.0, res.g - res.r) * GRN_YEL;

    // 2. Blue to Crisp Cyan/Sky Shift + Anti-Brown Shield
    float blueMask = max(0.0, res.b - res.g);
    res.g += blueMask * BLU_CYN;
    res.r -= blueMask * (BLU_CYN * 0.4);

    // 3. Calm & Tamed Red
    float redMask = max(0.0, res.r - max(res.g, res.b));
    res.r -= redMask * CLM_RED;
