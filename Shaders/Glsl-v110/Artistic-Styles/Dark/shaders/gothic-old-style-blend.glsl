/* RetroArch Gothic Crypt Enhanced - Halation with Threshold, Black Lift, Smooth Texture Fetch, Animated Film Grain */
#version 110
...
// Exact animated film noise from your original shader
...
void main() {
    ...
    vec3 halation_bleed =
        (texture2D(Texture, uv + texelSize * 1.5).rgb +
         texture2D(Texture, uv - texelSize * 1.5).rgb) * 0.5;
    ...
    vec3 gothicColor = vec3(final_luma) * vec3(0.8, 0.9, 1.25);
