#version 100
// تم رفع الدقة لضمان دقة النويز ومنع تكسره إلى مربعات
precision highp float;

// --- NTSC MEGA LITE (ULTRA-GRAIN MERGE) ---
...
#pragma parameter MD_JAILBARS "MD Vertical Jailbars" 0.15 0.0 1.0 0.01
...
uniform float CRAWL_SPEED, JITTER, MD_JAILBARS, JAIL_WIDTH;
#endif
...
// Hash عالي الدقة (Ultra-Grain Logic)
float hash(vec2 co) {
...
void main() {
    ...
    // 4. Final Luma & MD Jailbars (Resolution Independent)
    float final_y = mix(yM, (yL + yM + yR) * 0.3333, de_dither * dither_mask);
    
    if(MD_JAILBARS > 0.0) {
        float jailbar = sin(uv.x * 500.0 * (3.14159 / JAIL_WIDTH));
        final_y += jailbar * MD_JAILBARS * 0.02;
    }
