...
void main() {
    ...
    vec3 D = texture2D(Texture, uv+vec2(px.y,0.0)).rgb; // تم تصحيح الاتجاه هنا ليكون Y
    
    float lc = lum(C);
    
    // [1] حساب الـ AO (التعميق)
    // نقارن سطوع المركز بمتوسط سطوع الجيران
    float avgLum = lum((L + R + U + D) * 0.25);
    float ao = clamp((avgLum - lc) * AO_STR, 0.0, 1.0);
    
    vec3 col = C * (1.0 - ao); // تطبيق الـ AO بتعتيم المركز
    
    // [2] حساب الـ Edges (لـ Outline والـ Rim)
