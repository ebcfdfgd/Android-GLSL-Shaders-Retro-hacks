...
/* SUPER-RAINBOW-NTSC (Version 130 Build)
   - Integrated: Rotation Matrix for Rainbow Tilt.
   - Integrated: Signal Jitter, Mist, and Chroma Bleeding.
   - Optimized: Modern 'in/out' syntax for higher compatibility.
*/
...
void main() {
    ...
    // 1. JITTER (إهتزاز الإشارة)
    vec2 uv = TEX0;
    if(JITTER > 0.0) {
        uv.x += (rand(vec2(time, TEX0.y)) - 0.5) * JITTER * ps.x;
    }

    // 2. SIGNAL PROCESSING (معالجة الإشارة)
    ...
    // 3. CHROMA & HUE (اللون والصبغة)
    float bleed = ps.x * COL_BLEED;
    vec2 chrL = RGB_TO_YIQ(texture(Texture, uv - vec2(bleed, 0.0)).rgb).gb;
    vec2 chrR = RGB_TO_YIQ(texture(Texture, uv + vec2(bleed, 0.0)).rgb).gb;
    ...
    // 4. SHARPENING & MIST (الحدة والضبابية)
    float yC = res.r;
