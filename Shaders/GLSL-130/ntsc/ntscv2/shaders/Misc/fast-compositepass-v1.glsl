...
/*
    ...
    Reduced sharpness complexity while maintaining Artifacts & Tilt.
*/
...
#define saturate(c) clamp(c, 0.0, 1.0)
    ...
    void main()
    {
        // 1. حساب إحداثيات الـ Artifacts (سحبتان 2)
        ...
        vec4 art_b = tex2D(NTSCArtifactSampler2, scanuv + vec2(0.0, SourceSize.w));
        float lerpfactor = (animate_artifacts > 0.5) ? mod(float(FrameCount), 2.0) : NTSCLerp;
        vec4 NTSCArtifact = mix(art_a, art_b, 1.0 - lerpfactor);

        // 2. سحب عينات الصورة الأساسية (3 سحبات)
        vec2 offset_x = vec2(SourceSize.z, 0.0);
        ...
        // 4. الـ Sharpness المحسن (صفر سحبات إضافية - نستخدم ما لدينا)
        float curBrt = Brightness(Cur_Local);
        float NBrtL  = Brightness(Cur_Left);
        float NBrtR  = Brightness(Cur_Right);
        
        // حساب الحدة باستخدام العينات المسحوبة مسبقاً
        float sharp_diff = (curBrt * 2.0 - NBrtL - NBrtR);
        Cur_Local = saturate(Cur_Local + (sharp_diff * Tuning_Sharp * mix(vec4(1.0), NTSCArtifact, Tuning_Artifacts)));
