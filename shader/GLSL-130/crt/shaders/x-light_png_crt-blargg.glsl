...
/* ULTIMATE-OMNI-ENGINE (Hybrid 300 Optimized)
...
void main() {
    ...
    vec3 col_l = texture(Texture, final_uv - vec2(ps.x, 0.0)).rgb;
    vec3 col_r = texture(Texture, final_uv + vec2(ps.x, 0.0)).rgb;
    
    float bleed_off = ps.x * (COL_BLEED + fring * 2.0);
    vec3 cL = texture(Texture, final_uv - vec2(bleed_off, 0.0)).rgb;
    vec3 cR = texture(Texture, final_uv + vec2(bleed_off, 0.0)).rgb;
    ...
    res = clamp(res * res, 0.0, 1.0); // Linearize for Color Correction

    // --- [D] Color Profiles (Pro-E) ---
    if (CLU_PROFILE > 0.5 && CLU_PROFILE < 1.5) { /* EBU Neutral */ } 
    else if (CLU_PROFILE > 1.5 && CLU_PROFILE < 2.5) { res = clamp(res * mat3(0.95, 0.05, 0.0, 0.02, 0.98, 0.0, 0.0, 0.05, 0.95), 0.0, 1.0); } 
    else if (CLU_PROFILE > 2.5) { res = clamp(res * mat3(0.9, 0.1, 0.0, 0.05, 0.9, 0.05, 0.0, 0.1, 1.1), 0.0, 1.0); }

    // --- [E] CRT Adjustments: Contrast, Glow, Black Depth ---
    ...
    if (MASK_STR > 0.0) {
        ...
        } else mcol = texture(SamplerMask1, gl_FragCoord.xy / vec2(mw, mh)).rgb * 1.5;
    ...
    FragColor = vec4(sqrt(max(res, 0.0)), 1.0); // Gamma Out
}
#endif