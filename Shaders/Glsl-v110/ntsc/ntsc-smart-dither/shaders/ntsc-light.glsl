/* --- 777-NTSC-ULTRA-CLEAN-V3 (Fixed Precision) ---
    - FIXED: High-Precision RF Grain (Eliminates banding).
    - FIXED: Boiling Noise (Removed hard loop to prevent flicker).
    - FIXED: Jailbars are now resolution independent.
    - OPTIMIZED: G90T Stable Logic.
*/
...
#pragma parameter sig_noise "Signal RF Grain" 0.03 0.0 0.5 0.01
...
#ifdef GL_ES
// السر: رفع الدقة هو ما يمنع تكسر النمش
...
void main() {
    ...
    // Analog Artifact Injection
    float final_y = dot(col, kY);
    
    // النمش عائم (Floating) ومحمي بدقة highp
    if (sig_noise > 0.0) {
    ...
    if (jail_str > 0.0) {
        final_y += sin(vTexCoord.x * jail_width * 500.0) * jail_str * 0.05;
    }
