...
/*
    MEGA-HYBRID-ULTIMATE (V1.1 - Backported to 110)
    - Converted 'in/out' to 'attribute/varying'.
    - Replaced texture() with texture2D().
    - Added precision qualifiers for GL_ES.
*/
...
void main() {
    ...
    vec3 col_m = texture2D(Texture, f_uv).rgb * BRIGHT_BOOST;
    vec3 col_l = texture2D(Texture, f_uv - vec2(ps.x * de_dither, 0.0)).rgb * BRIGHT_BOOST;
    vec3 col_r = texture2D(Texture, f_uv + vec2(ps.x * de_dither, 0.0)).rgb * BRIGHT_BOOST;
    ...
    if (OverlayMix > 0.0) {
        vec3 l1 = texture2D(overlay, vec2(fract(mP.x * OutputSize.x / LUTWidth), fract(mP.y * OutputSize.y / LUTHeight))).rgb;
        res = mix(res, vec3(overlay_f(res.r, l1.r), overlay_f(res.g, l1.g), overlay_f(res.b, l1.b)), OverlayMix);
    }
    if (OverlayMix2 > 0.0) {
        vec3 l2 = texture2D(overlay2, vec2(fract(mP.x * OutputSize.x / LUTWidth2), fract(mP.y * OutputSize.y / LUTHeight2))).rgb;
