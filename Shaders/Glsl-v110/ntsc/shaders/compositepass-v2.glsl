// --- NTSC-ULTIMATE-PERSISTENCE (Brightness Build + RF Grain) ---
// - Cleaned: Removed Black Level, Resolution, and Sharpness controls.
// - Added: Global Signal Brightness control.
...
uniform float NTSC_BRIGHTNESS, ntsc_hue, SATURATION, fring, afacts;
...
void main()
{
    vec2 invSize = vec2(1.0 / TextureSize.x, 1.0 / TextureSize.y);
    ...
    float cosH = cos(hue_angle) * SATURATION;
    float sinH = sin(hue_angle) * SATURATION;
    vec2 rotated_chroma;
    ...
    if (sig_noise > 0.0) res += (hash(TEX0 + float(FrameCount) * 0.01) - 0.5) * sig_noise;
