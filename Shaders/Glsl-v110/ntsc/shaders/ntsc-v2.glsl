...
void main() {
    ...
    vec3 yiqL = (de_dither > 0.0) ? (cL * RGBtoYIQ) : (main_c * RGBtoYIQ);
    
    float lumaL = yiqL.r;
    float lumaR = (de_dither > 0.0) ? (texture2D(Texture, uv + vec2(ps.x * de_dither, 0.0)).rgb * RGBtoYIQ).r : yiq.r;
    ...
    if (COL_BLEED > 0.0 || red_persistence > 0.0) {
        ...
        // النشر العادي للكروما
        chroma = (yiq.gb + chrL + chrR) * 0.333; 

        // إضافة سيلان الأحمر جهة اليمين باستخدام قيمة I من اليسار
        if (red_persistence > 0.0) {
            float i_smear = mix(yiq.g, chrL.x, 0.4 * red_persistence);
            chroma.x = mix(chroma.x, i_smear, 0.6);
        }
    }
