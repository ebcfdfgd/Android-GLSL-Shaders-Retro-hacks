/* ULTIMATE SONIC 2026 - FINAL REMAKE ENGINE EDITION + BLOOM (OPTIMIZED) */
...
varying vec2 vec2 texCoord; uniform sampler2D Texture; uniform vec2 TextureSize;
uniform float dither_str, SCAN_STR, OUTLINE_STR, MICRO_AO, AO_SKIN_PROT, RIM_LIGHT, RIM_MASK_POWER, SHARPEN_STR, BLOOM_STR, DEPTH_FADE, LUMA_ADAPT, HIGH_COMP, VIBRANCE, WARMTH;
...
void main() {
    ...
    // [2] SHARPEN & ATMOSPHERIC DEPTH FADE
    vec3 sharpTarget = texture2D(Texture, texCoord + px).rgb;
    res = mix(res, res + (res - sharpTarget) * SHARPEN_STR, 0.5);
    // Adjusted depth fade to act as atmospheric fog rather than crushing blacks globally
    res *= (1.0 - (texCoord.y * DEPTH_FADE * 0.5));

    // [3] OUTLINE & FIXED VERTICAL RIM-LIGHT / AO
    float y_m = dot(res, lumaWeight);
    vec3 col_r = texture2D(Texture, texCoord + vec2(px.x, 0.0)).rgb;
    vec3 col_u = texture2D(Texture, texCoord + vec2(0.0, px.y)).rgb; // Fixed: True vertical sample
    float edge = dot(abs(res - col_r) + abs(res - col_u), vec3(0.333));
    
    // Rim Light Masking
    float rimMask = pow(clamp(1.0 - (y_m * RIM_MASK_POWER), 0.0, 1.0), 2.0);
    res += (edge * RIM_LIGHT * rimMask * 1.5 * OUTLINE_STR); 
    
    // Micro-AO with Skin Protection
    res -= (abs(dot(col_r, lumaWeight) - dot(col_u, lumaWeight)) * MICRO_AO * step(y_m, AO_SKIN_PROT));

    // [4] OMNIDIRECTIONAL 4-TAP BLOOM (Fixed Ghosting)
    vec3 bloom = texture2D(Texture, texCoord + vec2(px.x, 0.0) * 1.5).rgb +
                 texture2D(Texture, texCoord + vec2(-px.x, 0.0) * 1.5).rgb +
                 texture2D(Texture, texCoord + vec2(0.0, px.y) * 1.5).rgb +
                 texture2D(Texture, texCoord + vec2(0.0, -px.y) * 1.5).rgb;
    bloom *= 0.25; // Normalize bloom blur
    ...
    // [*] CINEMATIC AMBIENT INJECTION (Color Illusion Core)
