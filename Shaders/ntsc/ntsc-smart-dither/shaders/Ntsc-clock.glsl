...
#ifdef GL_ES
// Use highp to ensure grain is fine pixels rather than lines
...
void main() {
    ...
    // [1] Fetch center sample
    vec3 col_center = texture2D(Texture, pos).rgb;

    // --- Smart Bypass check for performance ---
    if (ARTF_STR <= 0.0 && RAINBOW_STR <= 0.0 && CHROMA_BLEED <= 0.0 && red_persistence <= 0.0 && SHARPNESS <= 0.0 && sig_noise <= 0.0) {
    ...
    // 3. Neighbor samples (L/R) for sharpness and bleeding
