/* 777-NTSC-MEGA-LITE-ULTRA-GRAIN
    ...
    - FEATURE: Clean MD Jailbars (Resolution Independent) and Rainbow Artifacts.
*/
...
#pragma parameter JAIL_WIDTH "Jailbar Spacing" 5.0 0.5 50.0 0.5
...
uniform float COL_BLEED, rb_power, rb_size, rb_detect, rb_speed, rb_tilt, de_dither, sig_noise, JAIL_STR, JAIL_WIDTH;
#endif
...
void main() {
    ...
    // التعديل هنا: تم إزالة TextureSize.x لجعل التردد مستقلاً عن الدقة
    if (JAIL_STR > 0.0) {
        final_y += sin(vTexCoord.x * JAIL_WIDTH * 100.0) * JAIL_STR * 0.02;
    }
