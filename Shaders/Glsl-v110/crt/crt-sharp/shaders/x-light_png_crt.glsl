...
/* ULTIMATE-TURBO-HYBRID-V6-QUILEZ-5050
...
void main() {
    ...
    vec2 i = floor(p);
    vec2 f = p - i;
    f = f * f * (3.0 - 2.0 * f); // Quilez smoothing
    
    vec3 res = texture2D(Texture, (i + f + 0.5) * inv_tex_size).rgb;
    
    // 3. Smart Scanlines (Branchless Mix)
    ...
    // حساب RGB Mask رياضياً للحالتين (الرفيع والعريض)
